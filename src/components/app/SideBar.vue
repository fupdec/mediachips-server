  ...
  <v-navigation-drawer app permanent mini-variant expand-on-hover clipped>
    <vuescroll :ops="ops">
      <v-list nav dense>
        ...
        <v-list-item v-for="meta in metaList" :key="meta.id"
        ...
        <div v-if="showHidden">
          <v-list-item v-for="meta in hiddenMetaList" :key="meta.id+1"
            link exact :to="`/meta/?metaId=${meta.id}&tabId=default`" color="secondary" draggable="false">
            <v-list-item-icon>
              <v-icon>mdi-{{meta.icon}}</v-icon>
            </v-list-item-icon>
  ...
  methods: {
    getMediaList() {
      axios.get(this.apiUrl + '/api/mediaTypes')
        .then(res => { this.mediaTypes = res.data })
        .catch(e => { console.log(e) })
    },
    getMetaList() {
      axios.get(this.apiUrl + '/api/meta')
        .then(res => { 
